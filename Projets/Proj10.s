...
Visit:
 	...
 	sw $fp, -12($sp) # Preserve the old frame pointer
	sw $ra, -16($sp) # Preserve the return adress
 	move $fp, $sp # Move the stack pointer into the frame pointer
	sw    $s0,-20($sp)
 	addiu $sp, $sp, -32 # Creating space on the stack pointer

 	lw $s0, -4($fp) # Puts x into $s0
 	lw $s1, -8($fp) # Puts y into $s1


	jal	XYToIndex # Jump and link to XYToIndex
  lw	$s2, -4($sp)	# Loads result into $s2
	la	$s3, grid	# Loads grid into $s3
	lb	$s4, SPACE	#  Loads space into $s4
	add	$s3, $s3, $s2  # Adds $s3 and $s2 to get the index
	sb	$s4, 0($s3)	# Puts ' ' into the current index
...
loopOneSetup:
	li  $t4,0
	forLoopOne:
	beq $t4,4,loopTwoSetup # If i>3
	jal rand
	sw	$t5, -4($sp)	# Store the result
	and $t5,$t5,3 # int r=rand()&3

	#     int temp = dirs[r];
	#     dirs[r] = dirs[i];
	#     dirs[i] = temp;
  # These should be the next three lines to be implemented
	# and with my current skill in MIPS, I do not have the
	# knowledge to do dirs[r] or dirs[i] without making
	# a large amount of conditional jumps as I did in the
	# switch below on line 319
	# Without these three lines, the visit function
	# should not and will not function properly

	add $t4,$t4,1
	j forLoopOne # Repeat loop
...
loopTwoSetup:
	...
	# Storing the old data for safety (probably unneeded)
	lw $t8,-8($sp)
	lw $t9,-12($sp)

	# if is in bounds x2 y2
	sw $t4,-8($sp)
	sw $t5,-12($sp)
	jal IsInBounds
	lw $t6,-4($sp)

	#Restoring old stack values
	sw $t8,-8($sp)
	sw $t9,-12($sp)

	beq $t6,0,loopTwoElse # If IsInBounds is 0

	ifNumberSign:
	jal XYToIndex
	lw	$s2, -4($sp)	# Loads result into $s2

	# From here I would need to check if the grid location
	# of $s2 was a '#' character
	# After doing this I would easily be able to begin the
	# recursive loop
	#
	# sw $t4,-4($fp)
	# sw $t5,-8($fp)
	# j Visit
  #
	# After These lines execute, the code above
	# will extract the new coordinates from the stack
	# I do not know if I should restore all the
	# saved values (frame pointer, return value,etc..)
